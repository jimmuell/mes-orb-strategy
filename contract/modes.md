...
`*` = declared for the vocabulary but **not implemented in v1** — a template requesting it triggers `UNSUPPORTED_CONSTRUCT` and enters the engine backlog.

## UNSUPPORTED_CONSTRUCT

When a field's `mode` is absent from the v1 vocabulary for its dimension (or is a `*` not-yet-implemented token), the mapper **fails fast** and returns (WIT-03 §3.7):
