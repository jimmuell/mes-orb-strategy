...
| WIT-P4o-report.md | Fix (bounded result payload) | Backtest payload emits a DAILY, hard-capped (≤5000) equity curve + equity_curve_resolution marker instead of the per-bar series (anchor 11.70 MB→0.130 MB, 90×); KPIs computed from full series untouched; fixed the Cloudflare 520/522 write failure; event-study payload found already bounded |
