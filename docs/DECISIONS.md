...
**Code anchors (as of current main — lines drift; the function names are the durable anchor):** in
`api/engine/engine.py`, `run_backtest` (def `:556`) and `run_backtest_long_short` (def `:919`); the
`data_first > start` guard at `:623` / `:1009`; the per-bar `start <= bar_date <= end` check at
`:674` / `:1055`; normalization applied just above each block.
